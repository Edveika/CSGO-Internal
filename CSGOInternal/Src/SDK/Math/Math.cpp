...
bool Math::WorldToScreen(Vector3 position, Vector2& screenPosition, float viewMatrix[16], LONG screenWidth, LONG screenHeight)
{
...
Vector3 Math::CalcAngle(Vector3 src, Vector3 dst)
{
	...
	float deltaLenght = sqrt(delta.x * delta.x + delta.y * delta.y + delta.z * delta.z);

	//result.x = -atan2f(dst.x - src.x, dst.y - src.y) / ((PI * 180.0f) + 180.0f);
	//result.y = asinf((dst.z - src.z) / Distance(src, dst)) * (180.0f / PI);

	result.x = -asin(delta.z / deltaLenght) * (180.f / PI);
	result.y = atan2(delta.y, delta.x) * (180.f / PI);
	result.z = 0.0f;

	return result;
}

Vector3 Math::Clamp(Vector3 angle)
{
	// normalize
